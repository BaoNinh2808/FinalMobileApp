...
public class PlaneRenderer {
  ...
  private final float[] planeColor = new float[] {1f, 1f, 1f, 0.2f};//0f,0f,0f,0f로 바꾸면 surface detection 표시 안나옴
  private final float[] planeColor_2 = new float[] {0f, 0f, 0f, 0f};//##바닥 색 변경
  ...
  public void drawPlanes(Collection<Plane> allPlanes, Pose cameraPose, float[] cameraPerspective) {
    ...
    for (SortablePlane sortedPlane : sortedPlanes) {
      ...
      // Set plane color. ############    planeColor >> planeColor_2로 변경 : 바닥 흰색 >> 무색으로 변경 ###################
      GLES20.glUniform4fv(lineColorUniform, 1, planeColor_2, 0);
      ...
      if(ARMainActivity.plane_check==0) {
