 ...
 */
package com.example.mobileappfinal.Business_layer.common.rendering;
...
public class ObjectRenderer {
  ...
  //###################################  base_color 객체 변수 추가 >> 흰색으로 ###################################
  private static final float[] base_color = new float[] {1f, 1f, 1f, 1f};
  //################################### 객체 회전을 위한 변수 declare and initialize a matrix (4 x 4 array). #####
  ...
  public void draw(float[] cameraView, float[] cameraPerspective, float[] colorCorrectionRgba) {
    draw(cameraView, cameraPerspective, colorCorrectionRgba, base_color);
  }
  ...
  public void draw(
    ...
    //region #################  texture 색깔 변경(139, 195, 74, 255 : 초록색) > (255,255,255,255) #################
    // Set the object color property. texture 색깔 변경 objColor변수(0,) >> base_color추가 RGBA(1f,1f,1f,1f)
    GLES20.glUniform4fv(colorUniform, 1, base_color, 0);
    //endregion
    ...
    // regionRotation을 위한 modelViewProjectionMatrix >> 변경 mFinalModelViewProjectionMatrix
    GLES20.glUniformMatrix4fv(modelViewUniform, 1, false, modelViewMatrix, 0);
    GLES20.glUniformMatrix4fv(modelViewProjectionUniform, 1, false, mFinalModelViewProjectionMatrix, 0);//ModelViewProjectionMatrix
    //endregion
    ...
    if (blendMode != null) {
      ...
      switch (blendMode) {
        case Shadow:
          ...
          GLES20.glDepthMask(false);// depth checking (false) : 뎁스적용X
          GLES20.glBlendFunc(GLES20.GL_ZERO, GLES20.GL_ONE_MINUS_SRC_ALPHA);
          break;
        case AlphaBlending:
          // Alpha blending function, with the depth mask enabled.
          GLES20.glDepthMask(true); // depth checking (true) : 뎁스적용O
          ...
          // GL_ZERO : 검정색, GL_ONE_MINUS_SRC_ALPHA : 투명색
